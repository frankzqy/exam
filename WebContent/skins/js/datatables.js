...
(function($){
	$.fn.dtable = function(options){
		options = $.extend(
				$.fn.dataTable.defaults,
				{
				   ...
				   "rowCallback": function(row,data,index){ //为每行的最后一个单元格绘制操作按钮
						...
						if(options.btnDefs != null && options.btnDefs != ""){
						   var operationBtn = table.format(data).html();
						   var operation = "<div class='operation'><span class='fa fa-angle-double-left'></span></div>";
						   if($lasttd.find(".compera").length == 0){  //防止前端分页时的重绘问题
							   $lasttd.append("<div class='compera'>"+operationBtn+operation+"</div>");
						   }
						}
				   }
				},
		...
		table.fold = function(){
			var that = $(this);
			that.children("tbody").on("click",".operation",function(){
				var $disCell = $(this).prev(".operationBtn");
		...
		table.format=function(data){
			...
			var containerBtn=$('<div class="btn-group operationBtn"></div>');
			for(var i = 0; i < options.btnDefs.length; i++){
				var item = $('<button class="btn ue-btn" type="button">' + options.btnDefs[i].text + '</button>');
