...
return null;}})();L5.Command=function(id){this.id=id;this.data="";this.params=new L5.Map("ParameterSet");this.returns=new L5.Map("ParameterSet");};L5.Command.context=new L5.Map();L5.Command.setContextParameter=function(name,value){L5.Command.context.put(name,value);}
...
frame.onload=function(){var r={responseText:'',responseXML:null};r.argument=o?o.argument:null;try{var doc;if(L5.isIE){doc=frame.contentWindow.document;}else{doc=(frame.contentDocument||window.frames[id].document);}
...
return{data:this.data,records:records};},handleFailure:function(jqXHR,textStatus,errorThrown){alert(errorThrown);},getData:function(model_descriptor){if(!model_descriptor){this.flatData=this.data;return this.flatData;}
...
return this.flatData;},getSingleData:function(model_descriptor){return this.getData(model_descriptor)[0];},getDataInfo:function(){return this.dataInfo;},getOriginalData:function(){return this.data;}};L5.DataConvert=function(orgData){var jsObj=eval(orgData);var data=[];for(var i=0;i<jsObj.length;i++){var obj=new Object();obj.javaClass="Record";obj.id=i;if(jsObj[i].state==1){obj.state=1;}else{obj.state=3;}
...
L5.Dataset=function(config){this.data=new L5.List();this.data.getKey=function(o){return o.id;};this.id;this.cmd;this.method;if(config.recordType){this.recordType=config.recordType;this.fields=this.recordType.prototype.fields;this.currentRecod=new this.recordType();}
...
sync=params.sync;}}
cmd.execute(this.method,sync,o);},loadRecords:function(o,options,success,scope){if(!o||success===false){alert("读取record对象失败！");return;}
var r=o.records,t=r.length;if(!options||options.add!==true){for(var i=0,len=r.length;i<len;i++){r[i].join(this);}
